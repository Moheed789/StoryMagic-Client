...
//   const [localModalStory, setLocalModalStory] =
//     useState<StoryDetails | null>(modalStory);
...
//       if (!resp.ok) {
//         throw new Error(respJson?.message || "You have used all 5 regenerations for this story.");
//       }
...
//           `${import.meta.env.VITE_BASE_URL}/stories/${storyIdFromPages}/pages/${pageNo}/image-status`,
//           { headers: { Authorization: `Bearer ${token}` } }
//         );
...
//           // update remaining regens (prefer backend value if present, otherwise just decrement)
//           setRegensLeft((prev) => {
//             if (typeof respJson?.remainingRegenerations === "number") {
//               return respJson.remainingRegenerations;
//             }
//             const currentLeft =
//               typeof prev === "number" ? prev : MAX_REGENERATIONS;
//             return currentLeft > 0 ? currentLeft - 1 : 0;
//           });
...
//             disabled={isBusy || regensLeft <= 0}
//           />

//           <div className="mt-4 flex items-center justify-between">
//             <span className="text-xs text-[#8C5AF2]">
//               {regensLeft > 0
//                 ? `${regensLeft} / ${MAX_REGENERATIONS} regenerations left`
...
//                 disabled={
//                   isBusy || !regenFeedback.trim() || regensLeft <= 0
//                 }
...
//                   {regensLeft <= 0 && (
//                     <div className="inline-block rounded-full bg-[#F3F4F6] text-[#4B5563] text-xs px-3 py-1">
//                       You&apos;ve used all 5 regenerations for this story.
//                     </div>
//                   )}
//                 </div>
...
const StoryPreviewModal: React.FC<StoryPreviewModalProps> = ({
  ...
  // 👉 ye naya derived number hai jo 1 se start hota hai
  const displayRegenNumber =
    regensLeft > 0 ? MAX_REGENERATIONS - regensLeft + 1 : MAX_REGENERATIONS;
      ...
      <div className="mt-6">
        ...
        <div className="">
          ...
          <div className="mt-4 flex items-center justify-between">
            <span className="text-xs text-[#8C5AF2]">
              {regensLeft > 0
                ? `${displayRegenNumber} / ${MAX_REGENERATIONS} regenerations left`
                : "You've used all 5 regenerations for this story."}
            </span>
            ...
            <div className="flex gap-3">
              ...
              <button
                type="button"
                onClick={handleRegenerateImage}
                disabled={isBusy || !regenFeedback.trim() || regensLeft <= 0}
                className="px-8 py-2 rounded-[6px] bg-[#8C5AF2] text-[12px] text-white font-semibold disabled:opacity-50"
              >
                {regenLoading ? "Applying..." : "Apply Now"}
              </button>
