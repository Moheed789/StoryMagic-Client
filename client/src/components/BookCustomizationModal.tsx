...
//    Ranges are inclusive and follow USPS general allocations. */
...
//   CT: [{ start: 6000, end: 6999 }], // 06000–06999 (leading 0)
...
//   ME: [{ start: 3900, end: 4999 }], // 03900–04999
//   MD: [{ start: 20600, end: 21999 }],
//   MA: [
//     { start: 1000, end: 2799 },
//     { start: 5500, end: 5599 },
//   ], // 01000–02799, 05500–05599
...
//   NH: [{ start: 3000, end: 3899 }], // 03000–03899
//   NJ: [{ start: 7000, end: 8999 }], // 07000–08999
...
//   RI: [{ start: 2800, end: 2999 }], // 02800–02999
...
//   VT: [{ start: 5000, end: 5999 }], // 05000–05999
//   VA: [
...
//   // handle leading-zero states by also trying number without leading zeros ranges mapped above
...
//   if (bookForm.shipping_address.postcode.length !== 5) {
//     toast({
...
//     if (zip.length === 5 && st) {
//       const ok = zipMatchesState(zip, st);
//       if (!ok) {
//         toast({
...
//     // Final guard before payment
//     if (!validateZipAgainstState()) {
//       return;
//     }
...
//                       });
//                     } else {
//                       validateZipAgainstState(); // your existing ZIP vs state check
//                     }
//                   }}
//                   inputMode="numeric"
//                   pattern="\d{5}"
...
const ZIP_RANGES: Record<string, Array<{ start: number; end: number }>> = {
  ...
  CA: [{ start: 90000, end: 96199 }],
  CO: [{ start: 80000, end: 81699 }],
  CT: [{ start: 6000, end: 6999 }], // 06000–06999
  DC: [{ start: 20000, end: 20599 }],
  DE: [{ start: 19700, end: 19999 }],
  FL: [{ start: 32000, end: 34999 }],
  ...
  ME: [{ start: 3900, end: 4999 }],  // 03900–04999
  MD: [{ start: 20600, end: 21999 }],
  MA: [{ start: 1000, end: 2799 }, { start: 5500, end: 5599 }], // 01000–02799, 05500–05599
  MI: [{ start: 48000, end: 49999 }],
  MN: [{ start: 55000, end: 56799 }],
  MS: [{ start: 38600, end: 39799 }],
  MO: [{ start: 63000, end: 65899 }],
...
const BookCustomizationModal: React.FC<BookCustomizationModalProps> = ({
  ...
  // city -> state helper for common cities
  const getStateFromCity = (city: string) => {
    const cityStateMap: Record<string, string> = {
      "New York": "NY",
      "Los Angeles": "CA",
  ...
  const handleCheckout = async () => {
    ...
    // exact 5 digits and correct state match required
    if (!validateZipAgainstState()) {
      return;
    }
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              ...
              <div>
                ...
                <input
                  ...
                  value={bookForm.shipping_address.postcode}
                  onChange={(e) => {
                    // only numbers, hard cap at 5 digits
                    const value = e.target.value.replace(/\D/g, "").slice(0, 5);
                    setBookForm((prev) =>
                      prev
                        ? {
                  ...
                  onBlur={() => {
                    // run exact-length and state match checks on field exit
