...
const BookCustomizationModal: React.FC<Props> = ({
  ...
  const [dynamicShippingPrice, setDynamicShippingPrice] = useState<
    number | null
  >(null);
  ...
  const shippingPrice = dynamicShippingPrice ?? ship.price;
  ...
  const handleCheckout = async () => {
    ...
    try {
      ...
      console.log("Address validation raw response:", data);
      ...
      const apiShippingCost = Number(
        data?.cost_estimate?.shipping_cost?.total_cost_incl_tax
      );

      if (!Number.isFinite(apiShippingCost)) {
        console.warn(
          "Invalid shipping cost from Lulu, falling back to static shipping price"
        );
        setDynamicShippingPrice(null);
      } else {
        setDynamicShippingPrice(apiShippingCost);
      }
  ...
  const handleContinueToPayment = async () => {
    ...
    try {
      ...
      toast({
        ...
        description: "Something went wrong while starting the payment.",
        variant: "destructive",
      });
    } finally {
      setLoading(false);
  ...
  return (
    <Modal
      isOpen={isOpen}
      onClose={onClose}
      title={step === 1 ? "Order your Printed Book" : "Book Summary"}
      ...
      <div className="pb-6 px-4 sm:px-6 space-y-5">
        {step === 1 && (
          <>
            <div className=" my-[31px]">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                {(["standard", "express"] as const).map((opt) => (
                  ...
                  >
                    <p className="text-[16px] font-story font-medium text-[#6F677E]">
                      {SHIPPING_PRICES[opt].label}
                    </p>
                    <div className="flex items-center gap-[12px] mt-[19px]">
                      <p className="text-[20px] font-semibold text-[#24212C]">
                    ...
                    <p className="text-[14px] font-medium text-[#6F677E] mt-[6px]">
            ...
            <div className="border-t border-slate-200 pt-[31px]">
                ...
                <div>
                  ...
                  <div className="flex items-center rounded-lg border border-slate-200 bg-white focus-within:ring-2 focus-within:ring-[#8C5AF2]/40">
                    ...
                    <input
                      type="tel"
                      value={formatUSPhone(
                        bookForm.shipping_address.phone_number
                      )}
                      onChange={(e) => {
            ...
            <button
              className="w-full h-11 rounded-xl bg-[#8C5AF2] text-white mt-5 font-semibold disabled:opacity-60"
              onClick={handleCheckout}
              disabled={loading}
            >
              {loading ? "Verifying Address..." : "Next"}
            </button>
          </>
        )}
      </div>
      {step === 2 && (
        <div className="px-4 sm:px-6 space-y-5 py-6 ">
          <div className=" rounded-xl border border-slate-200 bg-[#F5F5F5]">
          ...
          <button
            className="text-sm text-[#8C5AF2] underline mt-2"
            onClick={() => setStep(1)}
          >
            Back
          </button>
