...
//   const [downloadLoading, setDownloadLoading] = useState<{ [storyId: string]: boolean }>({});
...
//   const [previewsPurchased, setPreviewsPurchased] = useState<{ [storyId: string]: boolean }>({});
...
//   // Load stories + local preview state
//   useEffect(() => {
...
//             if (parsed && typeof parsed === "object") setPreviewsPurchased(parsed);
//           } catch {
...
//   // Verify Stripe session on return for preview unlock flow
//   useEffect(() => {
...
//           `${import.meta.env.VITE_BASE_URL}/stripe/verify-session?session_id=${encodeURIComponent(
//             sessionId
//           )}`,
...
//           toast({ title: "Unlocked", description: "Previews purchased successfully" });
//         }
...
//   // Verify Stripe session on return for normal purchases and unlock previews if pdf_only
//   useEffect(() => {
...
//             // Unlock preview because PDF Only was purchased
//             markPreviewPurchased(pending.storyId!);
//           }
//           await refetchStories();
//           toast({ title: "Purchase complete", description: "Your purchase has been applied" });
//         }
...
//     let finalPages = story.totalPages && story.totalPages > 0 ? story.totalPages : 0;
...
//         const res = await fetch(`${import.meta.env.VITE_BASE_URL}/stories/${story.storyId}`, {
//           headers: {
//             ...(token ? { Authorization: `Bearer ${token}` } : {}),
//             "Content-Type": "application/json",
//           },
...
//         if (res.ok) {
//           const data = await res.json();
//           if (data?.totalPages && data.totalPages > 0) finalPages = data.totalPages;
...
//       const response = await fetch(`${import.meta.env.VITE_BASE_URL}/stripe/story-download-session`, {
//         method: "POST",
//         headers: {
//           "Content-Type": "application/json",
...
//           downloadOption,
//           bookConfig: {
//             pageCount: extra?.pageOption ? Number(extra.pageOption) : undefined,
//             pagePrice: extra?.pageOption ? PAGE_PRICES[extra.pageOption] : undefined,
//             shippingMethod: extra?.shipping,
//             shippingPrice: extra?.shipping ? SHIPPING_PRICES[extra.shipping].price : undefined,
...
//       const checkoutUrl =
//         data.url || data.sessionUrl || data.checkout_url || data.checkoutUrl || data.session?.url;
...
//       } else {
//         toast({ title: "Checkout Error", description: "Checkout URL not found", variant: "destructive" });
//       }
//     } catch (error: any) {
//       toast({ title: "Purchase Failed", description: `Purchase failed: ${error.message}`, variant: "destructive" });
//     } finally {
//       setPurchaseLoading((prev) => ({
//         ...prev,
//         [storyId]: {
...
//       const res = await fetch(`${import.meta.env.VITE_BASE_URL}/stripe/story-download-book`, {
//         method: "POST",
...
//       }
//     } catch (error: any) {
//       toast({ title: "Checkout Failed", description: `Checkout failed: ${error?.message || "Unknown error"}`, variant: "destructive" });
//     } finally {
//       setPurchaseLoading((prev) => ({
//         ...prev,
...
//       const response = await fetch(`${import.meta.env.VITE_BASE_URL}/stories/${storyId}/export-pdf`, {
//         method: "GET",
...
//           toast({ title: "Download Started", description: "Your story download has begun" });
...
//         toast({ title: "Download Complete", description: "Your story has been downloaded successfully" });
//         setDownloadLoading((prev) => ({ ...prev, [storyId]: false }));
//         return;
//       }
...
//     } catch (error: any) {
//       toast({ title: "Download Failed", description: `Download failed: ${error.message}`, variant: "destructive" });
//     } finally {
//       setDownloadLoading((prev) => ({ ...prev, [storyId]: false }));
//     }
//   };
...
//         (item) => item.downloadOption === option && item.status === "paid" && item.downloadable === "Yes"
//       );
...
//     const purchased = previewsPurchased[storyId] === true;
...
//     } catch (err) {
//       console.error(err);
//       toast({ title: "Error", description: "Failed to load story preview", variant: "destructive" });
//     } finally {
...
//           <span className="text-[14px] text-green-600 font-medium">Previews Purchased ✓</span>
//         </div>
//       );
//     }
...
//         <div>
//           <span className="text-[14px] text-red-500 font-medium">Limit Reached</span>
//           <button
//             onClick={() => {
//               setUnlockModalStoryId(storyId);
//               setShowUnlockModal(true);
...
//     if (selectedDownloadOption[story.storyId]) return selectedDownloadOption[story.storyId];
...
//           ? { ...s, title: updatedStory.title, coverImageUrl: updatedStory.coverImageUrl }
//           : s
//       )
//     );
//     if (modalStory?.storyId === updatedStory.storyId) setModalStory(updatedStory);
//   };
...
//                   <p className="text-[14px] text-[#8C5AF2] italic mb-4 font-medium">Select Download Option</p>
...
//                           <span className="text-[14px] text-[#333333] font-medium">Downloadable PDF Only.</span>
//                           <span className="text-[14px] font-bold text-[#8C5AF2] ml-1">$2.99</span>
//                         </div>
//                       </div>
...
//                     <div
//                       onClick={() =>
//                         bookPurchased && handleDownloadOptionSelect(story.storyId, "pdf_and_book")
//                       }
//                       className={`flex items-center justify-between p-4 border rounded-[12px] transition-colors cursor-pointer ${
...
//                     <p className="text-[14px] text-[#8C5AF2] italic mb-6 font-medium">Purchase required to download</p>
//                   )}
...
//                       disabled={
//                         (!pdfPurchased && !bookPurchased) || downloadLoading[story.storyId]
//                       }
...
//             markPreviewPurchased(unlockModalStoryId);
//             setShowUnlockModal(false);
//             setUnlockModalStoryId("");
//           }
//         }}
...
const MyStories: React.FC = () => {
  ...
  // Centralized “previews unlocked” check
